...
contract Ebay{
    ...
    function createAuction(string calldata _name, string calldata _description, uint _minimum) external{
        require(_minimum > 0, "Minimum must be greater than 0");
        uint[] memory offerIds = new uint[](0);

        auctions[newAuctionId] = Auction(newAuctionId, payable(msg.sender), _name, _description, _minimum, 0, offerIds);
    ...
    function createOffer(uint _auctionId) external payable auctionExists(_auctionId){
        ...
        require(msg.value >= auction.minimum && msg.value > bestOffer.price, "msg.value must be greater than the minimum and the best offer");
